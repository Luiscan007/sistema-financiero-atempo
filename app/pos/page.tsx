...
function MetodoPagoForm({
    tipo, index, onActualizar, onEliminar, totalPendiente,
}: {
    ...
    const labels: Record<string, string> = {
        punto_venta: 'Punto de Venta', pago_movil: 'Pago Movil',
        efectivo_bs: 'Efectivo Bs', efectivo_usd: 'Efectivo USD',
        efectivo_eur: 'Efectivo EUR', transferencia: 'Transferencia',
    };
        ...
        <div className="bg-white/[0.03] border border-border rounded-xl p-4 space-y-3">
            <div className="flex items-center justify-between">
                <div className="flex items-center gap-2">
                    ...
                    <span className="text-sm font-medium">{labels[tipo] || tipo}</span>
                </div>
                ...
                <div className="grid grid-cols-2 gap-2">
                    <div>
                        ...
                        <select className="input-sistema text-xs" value={datos.banco || ''}
                            ...
                            {BANCOS_VENEZOLANOS.map((b: string) => <option key={b} value={b}>{b}</option>)}
                        </select>
                    </div>
                    <div>
...
export default function POSPage() {
    ...
    const serviciosFiltrados = todosServicios.filter((s: Servicio) => {
    ...
    const procesarVenta = async () => {
        ...
        if (totalPagado < total - 0.01) {
            toast.error('Falta pagar ' + fmtBs(total - totalPagado));
            return;
        }
        setProcesando(true);
    ...
    const enviarWhatsApp = () => {
        ...
        const numeroRecibo = generarNumeroRecibo();
        const lineas = items.map((item: any) => '- ' + item.nombre + ' x' + item.cantidad + ': ' + fmtBs(item.subtotal)).join('\n');
        const texto = 'RECIBO - ATEMPO ART STUDIO\n'
            + 'Recibo: ' + numeroRecibo + '\n'
            + 'Fecha: ' + new Date().toLocaleString('es-VE') + '\n\n'
            + 'SERVICIOS:\n' + lineas + '\n\n'
            + 'TOTAL: ' + fmtBs(total) + '\n'
            + '(aprox. ' + fmtUSD(totalUSD) + ')\n\n'
                    ...
                    {cargandoServicios ? (
                        ...
                        <div className="flex flex-col items-center justify-center h-full text-center text-muted-foreground py-16">
                            ...
                            <p className="text-xs max-w-xs">
                                Agrega servicios desde
                                <span className="text-blue-400 font-medium"> Inventario</span>
                            </p>
                        </div>
                        ...
                        <div className="grid grid-cols-2 sm:grid-cols-3 xl:grid-cols-4 gap-3">
                            {serviciosFiltrados.map((servicio: Servicio) => {
                                const precioBs = servicio.precioUSD * tarifaActual;
                                return (
                                    <button key={servicio.id} onClick={() => agregarAlCarrito(servicio)}
                                        ...
                                        <div>
                                            <p className="text-xs text-muted-foreground mb-0.5">
                                                {servicio.tipo === 'paquete_clases' ? 'Paquete' : 'Alquiler'}
                                            </p>
                                        ...
                                        <div className="mt-auto pt-2 border-t border-border/50">
                                            <p className="text-sm font-bold text-green-400 font-mono">{fmtUSD(servicio.precioUSD)}</p>
                                            <p className="text-xs text-amber-400 font-mono">{fmtBs(precioBs)}</p>
                                        </div>
                                    </button>
                                );
                            })}
                ...
                {ventaCompletada ? (
                    <div className="flex-1 flex flex-col items-center justify-center p-6 text-center">
                        ...
                        <p className="text-muted-foreground text-sm mb-6">
                            Total cobrado: {fmtBs(calcularTotal())}
                        </p>
                    ...
                    <>
                                ...
                                items.map((item: any) => (
                                    <div key={item.id} className="pos-item group">
                                        <div className="flex-1 min-w-0">
                                            ...
                                            <p className="text-xs text-muted-foreground">{fmtBs(item.precio)}</p>
                                        </div>
                                        <div className="flex items-center gap-2 flex-shrink-0">
                                            ...
                                            <div className="text-right min-w-[70px]">
                                                <p className="text-sm font-semibold font-mono">{fmtBs(item.subtotal)}</p>
                                                <p className="text-xs text-muted-foreground font-mono">{fmtUSD(item.subtotal / tarifaActual)}</p>
                                            </div>
                        ...
                        {/* Totales + Pago - scrolleable con altura maxima */}
                        {items.length > 0 && (
                            <div className="flex-shrink-0 border-t border-border flex flex-col" style={{ maxHeight: '60vh' }}>
                                <div className="overflow-y-auto p-4 space-y-4">
                                    ...
                                    <div className="space-y-1.5 text-sm">
                                        <div className="flex justify-between text-muted-foreground">
                                            <span>Subtotal</span>
                                            <span className="font-mono">{fmtBs(calcularSubtotal())}</span>
                                        </div>
                                        {descuentoGlobal > 0 && (
                                            <div className="flex justify-between text-yellow-400">
                                                <span>Descuento ({descuentoGlobal}%)</span>
                                                <span className="font-mono">-{fmtBs(calcularDescuento())}</span>
                                            </div>
                                        )}
                                        <div className="flex justify-between font-bold text-base border-t border-border pt-2">
                                            <span>TOTAL</span>
                                            <div className="text-right">
                                                <p className="font-mono text-blue-400">{fmtBs(calcularTotal())}</p>
                                                <p className="text-xs text-muted-foreground font-mono">
                                                    {fmtUSD(calcularTotal() / tarifaActual)} ({tasaSeleccionada.toUpperCase()})
                                                </p>
                                            </div>
                                        </div>
                                    </div>
                                    ...
                                    <div className="space-y-3">
                                        ...
                                        {metodoPago.map((metodo: any, index: number) => (
                                ...
                                {/* Confirmar - fijo al fondo, fuera del scroll */}
                                <div className="p-4 pt-2 flex-shrink-0 bg-card border-t border-border">
