 ...
 *                   ultimoMensaje, ultimoTimestamp, noLeidos{uid:n} }
 ...
 * agenda        → usa estructura de useAgenda:
 *                 { titulo, tipo:'reunion', estado:'confirmado',
 *                   fecha, horaInicio, horaFin, espacio, instructor,
 *                   descripcion, origen:'chat' }
 */
...
import {
    ...
    Download, ChevronRight, UserPlus, Circle,
} from 'lucide-react';
...
function fmtSep(ts: Timestamp): string {
    const d = ts.toDate(), hoy = new Date(), ayer = new Date();
...
function TarjetaEvento({ ev, esPropio }: { ev: EventoData; esPropio: boolean }) {
    return (
        <div className={cn(
            ...
            esPropio ? 'border-yellow-500/25 bg-yellow-500/6 rounded-tr-sm'
            ...
            <div className="px-3 py-2.5 space-y-1">
                ...
                <p className="text-[10px] text-white/55 flex items-center gap-1">
                    ...
                    {ev.fecha} · {ev.horaInicio} – {ev.horaFin}
                </p>
...
// ─── Speed Dial ───────────────────────────────────────────────────────────────
// FIX: posicionado ENCIMA del área de input, no superpuesto.
// Usamos un contenedor wrapper relativo en el input y el dial sale hacia arriba.
            ...
            {items.map((item, i) => (
                <div key={i} className={cn(
                    ...
                    <button onClick={() => { item.fn(); }}
                        className={cn('w-9 h-9 rounded-full flex items-center justify-center border shadow-lg transition-all hover:scale-110 active:scale-95', item.cls)}>
...
// FIX: posicionado encima del input con bottom: 100%

function PanelRapidas({ onSeleccionar, onCerrar }: {
    onSeleccionar: (t: string) => void; onCerrar: () => void;
}) {
    return (
        // bottom: 100% + mb-2 = sale justo encima del área de escritura
        <div className="absolute bottom-full left-0 right-0 mb-2 bg-[#111] border border-white/10 rounded-2xl shadow-2xl overflow-hidden z-30">
...
function ModalEvento({ onCerrar, onAgendar, usuarios, miUid }: {
    ...
    const toggle = (uid: string) => setForm(p => ({
        ...p,
        participantes: p.participantes.includes(uid)
                ...
                <div className="p-4 space-y-3 max-h-[65vh] overflow-y-auto">
                    <div>
                        ...
                        <input className="w-full bg-white/5 border border-white/10 rounded-xl px-3 py-2 text-sm text-white placeholder-white/20 outline-none focus:border-yellow-500/40"
                            placeholder="Reunion de staff, clase especial..."
                            value={form.titulo} onChange={e => set('titulo', e.target.value)} autoFocus />
                    </div>
                    <div>
                    ...
                    <div className="grid grid-cols-2 gap-2">
                        <div>
                            <label className="text-[10px] text-white/35 uppercase tracking-wider block mb-1">Hora inicio</label>
                        ...
                        <div>
                            <label className="text-[10px] text-white/35 uppercase tracking-wider block mb-1">Hora fin</label>
                            <input type="time" className="w-full bg-white/5 border border-white/10 rounded-xl px-3 py-2 text-sm text-white outline-none focus:border-yellow-500/40"
...
function ModalNuevoGrupo({ onCerrar, onCrear, usuarios, miUid }: {
    onCerrar: () => void;
    ...
    const toggle = (uid: string) => setSel(p => p.includes(uid) ? p.filter(u => u !== uid) : [...p, uid]);
            ...
            <div className="bg-[#111] border border-white/10 rounded-2xl w-full max-w-sm shadow-2xl">
                ...
                <div className="p-4 space-y-3">
                    <div>
                        ...
                        <input className="w-full bg-white/5 border border-white/10 rounded-xl px-3 py-2 text-sm text-white placeholder-white/20 outline-none focus:border-blue-500/40"
                            placeholder="Ej: Equipo Academia, Marketing..."
                    ...
                    <div>
                        <label className="text-[10px] text-white/35 uppercase tracking-wider block mb-2">
                            Miembros ({sel.length} seleccionados)
                        </label>
                ...
                <div className="flex gap-2 p-4 border-t border-white/8">
                    ...
                    <button onClick={() => {
                        ...
                        if (sel.length === 0) { toast.error('Selecciona al menos un miembro'); return; }
                        onCrear(nombre.trim(), sel);
                        onCerrar();
...
function PanelMiembros({ canal, usuarios, onCerrar, miUid, onAgregar }: {
    canal: Canal; usuarios: UsuarioOnline[];
    onCerrar: () => void; miUid: string;
    onAgregar: (uid: string) => void;
}) {
    ...
    return (
        <div className="flex flex-col h-full">
            ...
            <div className="flex-1 overflow-y-auto p-4 space-y-4">
                <div>
                    <p className="text-[10px] text-white/30 uppercase tracking-wider font-medium mb-2">En este canal</p>
                    <div className="space-y-1">
                        {miembros.map(u => (
                            <div key={u.uid} className="flex items-center gap-3 px-3 py-2.5 rounded-xl bg-white/3">
                                ...
                                <span className={cn('text-[10px] flex items-center gap-1 font-medium',
                ...
                {fuera.length > 0 && (
                    <div>
                        <p className="text-[10px] text-white/30 uppercase tracking-wider font-medium mb-2">Agregar al canal</p>
                        <div className="space-y-1">
                            {fuera.map(u => (
                                <button key={u.uid} onClick={() => onAgregar(u.uid)}
...
function VistaMensajes({ canal, miUid, miNombre, miRol, usuarios, onVolver }: {
    ...
    // Listener mensajes — sin orderBy para no requerir índice
    useEffect(() => {
    ...
    const handleArchivo = async (e: React.ChangeEvent<HTMLInputElement>) => {
        ...
        if (!file) return;
        if (file.size > 800 * 1024) { toast.error('Archivo supera 800KB. Comparte el link externo.'); return; }
        ...
        reader.onload = async ev => {
            await enviar(
                ...
                {
                    archivoUrl:    ev.target?.result as string,
    ...
    // FIX AGENDA: Guardar con la estructura exacta que lee useAgenda
    const handleAgendar = async (evData: EventoData) => {
        try {
            await addDoc(collection(db, 'agenda'), {
                // Campos requeridos por useAgenda / app/agenda/page.tsx
                titulo:       evData.titulo,
                tipo:         'reunion',          // TipoEvento válido en useAgenda
                estado:       'confirmado',
                fecha:        evData.fecha,
                horaInicio:   evData.horaInicio,  // ← antes era 'hora', ahora correcto
                horaFin:      evData.horaFin,      // ← campo nuevo obligatorio
                espacio:      '',
                instructor:   miNombre,            // quien agendó
                descripcion:  evData.descripcion,
                notas:        `Agendado desde chat. Participantes: ${evData.participantes.length + 1}`,
                montoBs:      0,
                montoUSD:     0,
                // Metadatos extra del chat (no afectan la visualización)
                origen:       'chat',
                canalId:      canal.id,
        ...
        toast.success('Reunion agendada ✅ — aparecera en la Agenda');
    };
    ...
    const handleAgregarMiembro = async (uid: string) => {
        ...
        toast.success(`${u.nombre} agregado`);
    };
    ...
    if (showMiembros) {
        return (
            <PanelMiembros
                canal={canal} usuarios={usuarios}
                onCerrar={() => setShowMiembros(false)}
                miUid={miUid} onAgregar={handleAgregarMiembro}
            />
        );
    }
    ...
    return (
        <div className="flex flex-col h-full">
            {/* Header canal */}
            <div className="flex items-center gap-3 px-4 py-3 border-b border-white/8 shrink-0">
                <button onClick={onVolver} className="p-1.5 hover:bg-white/8 rounded-xl text-white/45 hover:text-white">
                    <ArrowLeft className="w-4 h-4" />
                </button>
                {canal.tipo === 'dm' ? (
                    ...
                    <div className={cn('w-8 h-8 rounded-full flex items-center justify-center shrink-0',
                        canal.tipo === 'grupo'
                            ? 'bg-blue-500/12 border border-blue-500/22'
                ...
                {cargando ? (
                    ...
                    <>
                        {mensajes.map((m, i) => {
                            ...
                            const newDay = !prev || (m.timestamp && prev.timestamp &&
                                fmtSep(m.timestamp) !== fmtSep(prev.timestamp));
                            const mismo = prev?.autorId === m.autorId;
                            return (
                                <div key={m.id}>
                                    {(i === 0 || newDay) && m.timestamp && <SepFecha label={fmtSep(m.timestamp)} />}
                                    <Burbuja msg={m} esPropio={m.autorId === miUid} mostrarAvatar={!mismo} />
                                </div>
                            );
                        })}
                        <div ref={bottomRef} />
            ...
            {/* ── Zona de input — contenedor RELATIVO para los overlays ── */}
            <div className="shrink-0 border-t border-white/8">
                {/* Wrapper relativo: SpeedDial y PanelRapidas se posicionan respecto a este */}
                <div className="relative px-3 pb-3 pt-2">

                    {/* Speed Dial — sale hacia ARRIBA desde este contenedor */}
                    <SpeedDial
                        abierto={speedDial}
                    ...
                    {/* Panel respuestas rápidas — también sale hacia arriba */}
                    {showRapidas && (
                        <PanelRapidas
                    ...
                    <div className="flex items-end gap-2">
                        {/* Botón + */}
                        <button onClick={() => { setSpeedDial(p => !p); setShowRapidas(false); }}
                            className={cn(
                        ...
                        <textarea ref={inputRef} rows={1}
                            className="flex-1 bg-white/5 border border-white/10 rounded-2xl px-4 py-2.5 text-[13px] text-white placeholder-white/22 outline-none focus:border-white/18 resize-none leading-relaxed"
                        ...
                        <button onClick={() => enviar()} disabled={!texto.trim() || enviando}
                            className={cn(
                                ...
                                texto.trim() && !enviando
                                    ? 'bg-yellow-500/18 border-yellow-500/28 text-yellow-400 hover:scale-105'
...
function ListaCanales({ canales, usuarios, miUid, miNombre, onSeleccionar, onNuevoGrupo }: {
    canales: Canal[]; usuarios: UsuarioOnline[];
    miUid: string; miNombre: string;
    ...
    const sinDM = usuarios.filter(u =>
        u.uid !== miUid && !canales.some(c => c.id === dmId(miUid, u.uid))
    );
        ...
        <div className="flex flex-col h-full">
            <div className="px-3 pt-3 pb-2 space-y-2">
                <div className="relative">
                    ...
                    <input className="w-full bg-white/5 border border-white/8 rounded-xl pl-9 pr-3 py-2 text-[12px] text-white placeholder-white/22 outline-none focus:border-white/18"
                ...
                <button onClick={onNuevoGrupo}
                    ...
                    <UserPlus className="w-3.5 h-3.5" /> Crear nuevo grupo
                </button>
            </div>

            <div className="flex-1 overflow-y-auto">
                {filtrados.map(canal => {
                    const noL  = canal.noLeidos?.[miUid] ?? 0;
                        ...
                        <button key={canal.id} onClick={() => onSeleccionar(canal)}
                            ...
                            <div className="flex-1 min-w-0">
                                <div className="flex items-center justify-between mb-0.5">
                                    <span className={cn('text-[13px] font-medium truncate', noL > 0 ? 'text-white' : 'text-white/65')}>
                                        {nombre}
                                    </span>
                                    {canal.ultimoTimestamp && (
                                        <span className="text-[10px] text-white/22 shrink-0 ml-2">{fmtHora(canal.ultimoTimestamp)}</span>
                                    )}
                                </div>
                ...
                {sinDM.length > 0 && (
                    <>
                        <div className="px-4 pt-3 pb-1">
                            <p className="text-[10px] text-white/22 uppercase tracking-wider font-medium">Mensaje directo</p>
                        </div>
                        {sinDM.map(u => (
                            <button key={u.uid} onClick={() => onSeleccionar({
                                id: dmId(miUid, u.uid), tipo: 'dm',
            ...
            snap => {
                ...
                if (!general) {
                    setDoc(doc(db, 'chat_canales', 'general'), {
                        tipo: 'general', participantes: [miUid],
            ...
            <div className={cn(
                ...
                <div className="flex items-center justify-between px-4 py-4 border-b border-white/8 shrink-0 bg-black/30 backdrop-blur-xl">
                    <div className="flex items-center gap-2.5">
                        ...
                        <div>
                            <h2 className="text-[13px] font-semibold text-white">Mensajes</h2>
                            <p className="text-[10px] text-white/30">
                ...
                <div className="flex-1 overflow-hidden">
                    {cargandoInit ? (
                        ...
                        <VistaMensajes
                            canal={canalActivo} miUid={miUid}
                            miNombre={miNombre} miRol={miRol}
                            usuarios={usuarios}
                            onVolver={() => setCanalActivo(null)}
                        />
                    ) : (
                        <ListaCanales
                            canales={canales} usuarios={usuarios}
